...
if __name__ == '__main__':
    ...
    # Regression
    num_nodes = 300 # Number of enhancement nodes.
    regular_para = 0.0001 # Regularization parameter.
    weight_random_range = [-1, 1]  # Range of random weights.
    bias_random_range = [0, 1]  # Range of random weights.
    num_layer = 8  # Number of hidden layers

    list_metrics = np.zeros(10)
    weight_random = [random.uniform(weight_random_range[0], weight_random_range[1]) for _ in range(num_nodes)]
    bias_random = [random.uniform(bias_random_range[0], bias_random_range[1]) for _ in range(num_nodes)]

    deep_rvfl = EnsembleDeepRVFL(n_nodes=num_nodes, lam=regular_para, w_random_vec_range=weight_random_range,
                        b_random_vec_range=bias_random_range, activation='relu', n_layer=num_layer, same_feature=False,
                        task_type='regression', seed = 42)
    deep_rvfl.train(x_train, y_train,0)
    prediction = deep_rvfl.predict(x_test)
    prtrain= deep_rvfl.predict(x_train)

    # metrics
    std_train = y_train.std()
    std_test = y_test.std()
    r2_RVFL=r2_score(y_test, prediction)
    r = np.sqrt(r2_RVFL)
    r2_RVF_train=r2_score(y_train, prtrain)
    r_train = np.sqrt(r2_RVF_train)
    mse = mean_squared_error(y_test, prediction)
    mse_train= mean_squared_error(y_train, prtrain)
    mape = np.mean(np.abs((y_test - prediction) / y_test))
    mape_train = np.mean(np.abs((y_train - prtrain) / y_train))
    mean_y_true = np.mean(y_test)
    mean_y_true_train = np.mean(y_train)
    mae_test = mean_absolute_error(y_test, prediction)
    mae_train = mean_absolute_error(y_train, prtrain)
    nse = 1 - (np.sum((y_test - prediction.flatten()) ** 2) / np.sum((y_test - mean_y_true) ** 2))
    nse_train = 1 - (np.sum((y_train - prtrain.flatten()) ** 2) / np.sum((y_train - mean_y_true_train) ** 2))
    list_metrics = [r_train, r, mse_train, mse, mape_train, mape, nse_train, nse, mae_train, mae_test]
    
    print("R Train: %f" % (list_metrics[0]))
    print("R Test: %f" % (list_metrics[1]))
    print("MSE Train: %f" % (list_metrics[2]))
    print("MSE Test: %f" % (list_metrics[3]))
    print("MAPE Train: %f" % (list_metrics[4]))
    print("MAPE: %f" % (list_metrics[5]))
    print("NSE Train: %f" % (list_metrics[6]))
    print("NSE: %f" % (list_metrics[7]))
    print("MAE Train: %f" % (list_metrics[8]))
    print("MAE: %f" % (list_metrics[9]))
